...
- DATE ATTRIBUTION. A date is only correct if it is tied to the RIGHT event. Company events and person events are separate facts even when they share a company. A funding round, an acquisition, or a company announcement is a COMPANY date. A person's appointment, hire, promotion, or role change is a PERSON date. Do NOT attach a company date to a person, or a person date to a company, unless a single source explicitly links them (for example, one article that says the person was appointed as part of that funding round). If you find "April 2025" attached to an investment and you are writing about when the person took their role, do NOT reuse that date for the appointment unless a source says so. When in doubt about which event a date belongs to, state the date with the event you actually sourced it to ("the company announced Ancor's investment in April 2025"), not bound to the person. If you are inferring or estimating a person's start date or appointment date rather than reading it directly, either leave it out or mark it clearly as unconfirmed ("appointed President, date not confirmed").
...
DATES: a date about the person (when they were appointed, hired, or promoted) is only a fact if the Matrix cell states it about THAT PERSON. Do not borrow a company date (a funding round, an acquisition) and present it as the person's start or appointment date. If the Matrix ties a date to a company event, keep it attached to that event, not to the person. Only when you are clearly reaching for a person-level date that the Matrix does not cleanly support, add a short "worth confirming" note. Do not flag dates that are cleanly sourced; over-flagging trains the rep to ignore the flag, so keep it rare and meaningful.
